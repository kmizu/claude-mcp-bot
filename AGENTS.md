...
- 2026-02-07: Stabilized desktop reply-composer visibility across short-height/zoomed viewports by capping message-list growth (`max-height`) and keeping the chat panel anchored, then verified on production with Playwright screenshots at multiple viewport sizes and deployed cache bump (`v10`, `20260207i`).
- 2026-02-07: Performed full layout rethink for reply-field disappearance: removed root-level scroll lock, switched back to natural page scrolling with sticky header, rebalanced message-list max-heights for desktop/mobile, and verified full-page reachability on production in multiple low-height viewports after deploying cache bump (`v11`, `20260207j`).
- 2026-02-07: Investigated Pixel-class mobile rendering anomaly (`ss.png`, vertical text/reader-like layout), then added explicit horizontal writing-mode guards plus `notranslate` hints on `html/body` and redeployed with cache bump (`v12`, `20260207k`); production Pixel emulation now renders normal horizontal UI.
